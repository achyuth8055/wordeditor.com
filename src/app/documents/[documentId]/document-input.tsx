...
export const DocumentInput = ({ title, id }: DocumentInputProps) => {
  ...
  const debounceUpdate = useDebounce((newValue: string) => {
    ...
    if (newValue.trim() === '') {
      toastWrapper(() => toast.error('Title cannot be empty'));
      debounceUpdate('Untitled Document');
      setIsEditing(false);
      return;
    }
    ...
    mutate({ id, title: newValue })
      .then(() => toastWrapper(() => toast.success('Document updated')))
      .catch(() => toastWrapper(() => toast.error('Update failed')))
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    mutate({ id, title: value })
      .then(() => {
        toastWrapper(() => toast.success('Document updated'));
        setIsEditing(false);
      })
      .catch(() => toastWrapper(() => toast.error('Update failed')))
      .finally(() => setIsPending(false));
