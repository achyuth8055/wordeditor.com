...
/**
 * Extend the type definitions of the @tiptap/core module
 * Add fontSize related commands to the Commands interface
 */
declare module '@tiptap/core' {
  interface Commands<ReturnType> {
    fontSize: {
      /**
       * Set font size
       * @param options.size - Font size value, e.g.: '16px'
       */
...
/**
 * Configuration options interface for font size extension
 */
export interface FontSizeOptions {
  // Define which types of nodes can apply font size
  types: string[];
  // Add minimum and maximum font size limits
  minSize: number;
  maxSize: number;
  // Default font size
  defaultSize: number;
...
export const FontSizeExtension = Extension.create<FontSizeOptions>({
  // Unique name of the extension, used when registering and referencing the extension in the editor
  name: 'fontSize',

  /**
   * Configure default options for the extension
   * @returns Default configuration object
   *
   * Define default configuration options for the extension, when custom extension behavior is needed
   */
  addOptions() {
    return {
      types: ['textStyle'],
  ...
  /**
   * addGlobalAttributes is a configuration function in Tiptap for defining global attributes, returns an array for defining
   * how to parse and render font size attributes for multiple different node types
   * @returns Global attributes configuration array
   */
  addGlobalAttributes() {
    return [
      {
